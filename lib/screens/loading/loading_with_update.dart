...
class LoadingWithUpdateScreen extends StatefulWidget {
  const LoadingWithUpdateScreen({Key? key}) : super(key: key);

  @override
  _LoadingWithUpdateScreenState createState() {
    return _LoadingWithUpdateScreenState();
  }
}

class _LoadingWithUpdateScreenState extends State<LoadingWithUpdateScreen> {
  double _opacity = 0.0;
  double? _percent;
  ...
  void initState() {
    ...
    AppBloc.applicationCubit.progress = (received, total) {
      setState(() {
        _percent = received / total;
      });
    };
    AppBloc.applicationCubit.onLoading();
  }
                ...
                children: [
                  SizedBox(
                    ...
                    child: CircularProgressIndicator(
                      ...
                      strokeWidth: 2,
                      color: Colors.black,
                      value: (_percent ?? 0) > 0 ? _percent : null,
                    ),
                  ),
                  Text('يرجى الانتظار .. يتم تحديث البيانات',
